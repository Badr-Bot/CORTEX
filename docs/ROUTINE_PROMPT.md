...
LE LUNDI (date -u +%u = 1) — passe hebdomadaire, RADAR.md §6bis : scan élargi (G page 2, Triple Whale shopify_app_ids=[2982] J-120 min 50 ads, croissance 30 jours ads_growth last30d ≥ 100 % sort_by=growth30d) ; rafraîchis chaque boutique de data/radar/base_winners.json avec search_shops query=<domaine> match_mode=exact limit=1 (~2 crédits chacune, enregistre chaque réponse dans data/radar/raw/AAAA-MM-JJ-<domaine>.json) puis python -m agents.radar_produits extract AAAA-MM-JJ et python -m agents.base_winners refresh AAAA-MM-JJ ; relis les verdicts de Badr : avec le connecteur Google Drive (ToolSearch "select:mcp__Google-Drive__search_files,mcp__Google-Drive__download_file_content,mcp__Google-Drive__create_file"), trouve le dernier *-BASE-WINNERS.xlsx du dossier 1nH38fj8BsTF8DZ0KKYeZzKA5mmhSG3-B, télécharge-le dans data/radar/drive_BASE-WINNERS.xlsx et lance python -m agents.base_winners import-verdicts data/radar/drive_BASE-WINNERS.xlsx ; choisis jusqu'à 10 produits de la semaine qui passent tous les filtres pour radar_produits (enquête complète sur les 3 meilleurs, fiche courte pour les autres) ; puis python -m agents.base_winners export et dépose data/radar/BASE-WINNERS.xlsx sur le Drive avec create_file (base64Content, contentMimeType application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, disableConversionToGoogleType=true, titre AAAA-MM-JJ-BASE-WINNERS.xlsx, parentId 1nH38fj8BsTF8DZ0KKYeZzKA5mmhSG3-B). Les autres jours : rien de tout ça, la publication ajoute les pépites du jour à la base toute seule.
