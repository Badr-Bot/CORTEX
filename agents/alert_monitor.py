...
Scoring  : Groq Llama 3.3 gratuit — seuil 7/10 pour alerte
"""
...
# Mots-clés qui déclenchent le scoring Groq (évite d'appeler l'API pour chaque news)
URGENT_KEYWORDS = [
    # Géopolitique
    "war", "attack", "invasion", "missile", "nuclear", "explosion", "coup",
    "assassination", "troops", "conflict", "strike", "bombing", "guerre",
    "attaque", "invasion", "missile", "nucléaire",
    # Marchés / crise
...
async def _score_and_translate_groq(title: str, summary: str) -> tuple[int, str, str]:
    """
    Retourne (score, titre_fr, résumé_fr).
    Score 1-10 : 9-10 = conséquences macro/micro majeures, guerre, crash, décision Fed critique.
...
async def check_and_send_alerts() -> list[str]:
    ...
    try:
        ...
        for entry in all_entries:
            ...
            # Filtre rapide par mots-clés avant d'appeler Groq
